...
def verify(manifest, schema):
    ...
    if not schema:
        ...
        if not schema_path:
            click.echo("Error: Could not find skills-schema.json automatically. Please provide --schema.")
            return
    else:
...
@cli.command()
@click.option('--target', required=True, type=click.Choice(['claude', 'gemini', 'opencode']), help='Target harness to deploy to')
@click.option('--manifest', default='skills.json', help='Path to manifest file')
