...
class ClaudeDeployment(BaseDeployment):
    def deploy(self) -> bool:
        """Sync to .claude/ or .claudecode/ for Claude Code."""
        # Detect which directory to use, default to .claude
        target_root = self.root_dir / ".claude"
        if (self.root_dir / ".claudecode").exists() and not target_root.exists():
        ...
        for subagent in self.manifest.subagents:
            ...
            # Ensure the frontmatter is compatible with Claude
            try:
        ...
        self.log(
            f"Claude Deployment Summary: {skills_copied} skills, {agents_deployed} agents."
        )
        return True
